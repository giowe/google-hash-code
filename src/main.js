...
for(let r = 0; r < (argv.R || R); ++r){
  ...
  const endp = endpoints[ sortedReq[r].endpointId ]
  const cn = endp.cachesLength
  for(let c = 0; c < cn; ++c){
    const cache = endp.cacheLatencies[c]
  ...
  if(!endpoints[ ca.endpoint ].hasVideo(ca.video)){
    ...
    if(!cacheServers[ ca.cache ].hasVideo(ca.video) &&
      ...
      for(let e = 0; e < cacheServers[ ca.cache ].endpoints.length; ++e){
        const e_id = cacheServers[ ca.cache ].endpoints[e]
        const latency = endpoints[ e_id ].getCacheServer(ca.cache).latency// endpoints[ e_id ].cacheLatencies;
